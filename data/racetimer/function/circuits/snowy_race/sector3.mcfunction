...
# Check personal best
execute unless score @s pb_s3_snowy_race matches 1.. run scoreboard players set @s pb_s3_snowy_race 999999
execute if score @s tempo < @s pb_s3_snowy_race run scoreboard players set #is_pb costanti 1

# Check world record
execute store result score wr_time costanti run data get storage racetimer:snowy_race records.s3
execute if score @s tempo < wr_time costanti run scoreboard players set #is_wr costanti 1

# === SALVA VECCHIO WR PER CONFRONTO ===
execute if score #is_wr costanti matches 1 store result score old_wr costanti run data get storage racetimer:snowy_race records.s3
...
execute if score #is_wr costanti matches 1 store result storage racetimer:snowy_race records.s3 int 1 run scoreboard players get @s tempo
execute if score #is_wr costanti matches 1 as @s run function racetimer:core/save_player_name
execute if score #is_wr costanti matches 1 run data modify storage racetimer:snowy_race records.s3_player set from storage racetimer:temp player_name

# === SALVA VECCHIO PB PER CONFRONTO ===
execute if score #is_pb costanti matches 1 run scoreboard players operation old_pb costanti = @s pb_s3_snowy_race
...
# Aggiorna il PB
execute if score #is_pb costanti matches 1 run scoreboard players operation @s pb_s3_snowy_race = @s tempo
...
# Update tags
tag @s add settore3_snowy_race
tag @s remove settore2_snowy_race
